  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl overflow-x-auto shadow-sm border border-sage-50">
        <table className="w-full text-sm text-left whitespace-nowrap"><thead className="bg-sage-50 text-sage-900"><tr><th className="p-4">Nama</th><th className="p-4">Pihak</th><th className="p-4">Hubungan</th><th className="p-4">Alamat</th><th className="p-4">Status Undangan</th><th className="p-4 text-center">Aksi</th></tr></thead>
          <tbody className="divide-y divide-gray-100">
            {guests?.map(g => (
              <tr key={g.id} className="hover:bg-gray-50"><td className="p-4 font-medium text-sage-900">{g.nama}</td><td className="p-4 font-semibold text-xs text-sage-600">{g.pihak}</td><td className="p-4">{g.hubungan}</td><td className="p-4 text-gray-500">{g.alamat}</td>
                <td className="p-4"><select value={g.status} onChange={(e) => handleStatusChange(g, e.target.value)} className={`p-2 rounded-lg text-xs font-semibold ${g.status === 'Sudah Dikirim' ? 'bg-sage-100 text-sage-900' : 'bg-amber-50 text-amber-600'}`}><option>Belum Dikirim</option><option>Sudah Dikirim</option><option>Hadir</option></select></td>
                <td className="p-4"><div className="flex justify-center gap-3"><button onClick={() => handleEdit(g)} className="text-gray-400 hover:text-blue-500"><Edit size={18} /></button><button onClick={() => handleDelete(g.id)} className="text-gray-400 hover:text-red-500"><Trash2 size={18} /></button></div></td>
              </tr>
            ))}
          </tbody>
        </table>
